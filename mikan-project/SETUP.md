...
```yaml
ports:
  - "8081:80"  # 現在は8081に設定済み（MySQLは3307に設定済み）
```
...
**注意**: Dockerコンテナ内では`DB_HOST=mysql`、`DB_PORT=3306`を使用します。
ホストマシンから直接接続する場合は`DB_HOST=localhost`、`DB_PORT=3307`を使用してください。
