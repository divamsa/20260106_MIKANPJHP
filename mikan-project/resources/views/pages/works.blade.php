...
<style>
    ...
    .work-card {
        display: flex;
        flex-direction: column; /* モバイル：縦（上画像・下テキスト） */
    }

    @media (min-width: 768px) {
        .work-card {
            flex-direction: row;      /* PC：左画像・右テキスト */
            align-items: stretch;
            max-width: 1000px;
            margin: 0 auto;
        }
                                ...
                                <article class="work-card">
                                    @if($work->image_path)
                                        <div class="work-card__image-wrapper">
                                            <img
                                                src="{{ asset($work->image_path) }}"
                                                alt="{{ $work->title }}"
                                                class="work-card__image"
                                                loading="lazy"
                                            >
                                            <div class="work-card__overlay" aria-hidden="true"></div>
                                        </div>
                                    @endif
